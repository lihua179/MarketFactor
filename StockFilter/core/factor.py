...
def _daily_vol(data: Dict[str, List], n: int = 30,*params,**kwargs) -> Dict[str, Any]:
    high, low, yd_close = data['high'][-n:], data['low'][-n:], data['yd_close'][-n:]
    vols = [(h - l) / yc for h, l, yc in zip(high, low, yd_close) if yc]
    return {"res": sum(vols) / len(vols) if vols else 0.0}

def _price_level(data: Dict[str, List], n: int = 30,*params,**kwargs) -> Dict[str, Any]:

    high = max(data['high'][-n:])
    low = min(data['low'][-n:])
    close = data['close'][-1]
    return {"res": (close - low) / (high - low)}
# ---------- 1. 因子命名空间 ----------
class Factor:

    # 配置工厂
    @staticmethod
    def price_level(n: int = 30,rules: Rules = ("res > 0",), name: Optional[str] = None) -> FactorConf:
        return _price_level, {"n": n}, rules, name

    @staticmethod
    def daily_vol(n: int = 30, rules: Rules = ("res < 0.05",), name: Optional[str] = None) -> FactorConf:
