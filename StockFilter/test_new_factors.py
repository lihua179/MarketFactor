...
from core.factor_chain import Chain
...
def test_basic_statistics():
    ...
    if report['pass']:
        ...
        print(f"\n  结果:")
        print(f"    相对分位: {details['level_rel']['res']:.4f}")
        print(f"    绝对分位: {details['level_abs']['res']:.4f}")
        print(f"    距最高价%: {details['dist_high']['res']:.2f}%")
        print(f"    距最低价%: {details['dist_low']['res']:.2f}%")
...
def test_drawdown_indicators():
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    相对回撤: {details['dd_rel']['res']:.4f}")
        print(f"    绝对回撤: {details['dd_abs']['res']:.4f}")
        print(f"    回撤金额: {details['dd_amt']['res']:.2f}")
...
def test_volatility_and_returns():
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    日波动率: {details['vol_daily']['res']:.6f}")
        print(f"    期间波动率: {details['vol_period']['res']:.6f}")
        print(f"    总收益率: {details['ret_total']['res']:.4f}")
        print(f"    平均日收益率: {details['ret_avg']['res']:.6f}")
        print(f"    年化收益率: {details['ret_ann']['res']:.4f}")
...
def test_price_statistics():
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    平均价: {details['price_avg']['res']:.2f}")
        print(f"    中位数: {details['price_med']['res']:.2f}")
        print(f"    众数: {details['price_mode']['res']:.2f}")
        print(f"    极差: {details['price_range']['res']:.2f}")
        print(f"    标准差: {details['price_std']['res']:.4f}")
        print(f"    变异系数: {details['price_cv']['res']:.4f}")
...
def test_trend_indicators():
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    趋势方向: {details['trend_dir']['res']}")
        print(f"    趋势强度: {details['trend_str']['res']:.4f}")
...
def test_advanced_features():
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    躁动程度值: {details['agit_level']['res']:.6f}")
        print(f"    躁动评分: {details['agit_score']['res']}")
        print(f"    累积波动率: {details['cum_vol']['res']:.6f}")
...
def test_wash_trading():
    ...
    report = Config(
        AdvancedFeatures.is_wash_trading(agitation_threshold=0.2, rules=("res == True or res == False",), name='is_wash'),
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    是否洗盘: {details['is_wash']['res']}")
        print(f"    洗盘类型: {details['wash_type']['res']}")
        print(f"    洗盘强度: {details['wash_intensity']['res']:.6f}")
...
def test_combined_usage():
    ...
    report = Chain(
        # 基础指标
        BasicStatistics.level_relative(rules=("res >= 0",), name='price_level'),
        BasicStatistics.trend_direction(name='trend'),
        BasicStatistics.return_total(rules=("res > -0.5",), name='total_return'),

        # 高级特征
        AdvancedFeatures.agitation_level(rules=("res < 0.5",), name='agitation'),
        AdvancedFeatures.is_wash_trading(agitation_threshold=0.2, rules=("res == True or res == False",), name='is_wash')
    ).run(data)
    ...
    if report['pass']:
        details = report['details']
        print(f"\n  结果:")
        print(f"    价格水平: {details['price_level']['res']:.2%}")
        print(f"    趋势: {details['trend']['res']}")
        print(f"    总收益率: {details['total_return']['res']:.2%}")
        print(f"    躁动程度: {details['agitation']['res']:.4f}")
        print(f"    是否洗盘: {details['is_wash']['res']}")
